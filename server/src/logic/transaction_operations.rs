...
pub async fn delete_transaction_by_id(
	...
	transaction::Entity::delete_by_id(transaction_id).exec(conn).await?;
	Ok(())
}
...
async fn get_expense_by_id_if_exists(
	...
	match expense.await? {
		Some(res) => Ok(res),
		None => Err(
			ExpenseTransactionError::InvalidTransactionData(String::from(
				"We have not found the referred expense for the transaction",
			))),
	}
}
...
async fn validate_referred_currency_type(
	...
	if referred_currency_type.await?.is_none() {
			return Err(ExpenseTransactionError::InvalidTransactionData(String::from(
					"We have no currency type with the given id.",
			)));
	};
	Ok(())
}
