...
# Account activation remover scheduled event
SET GLOBAL event_scheduler = ON;
CREATE EVENT veryrezsi.account_activation_remover
    ON SCHEDULE
		EVERY 1 DAY
    COMMENT 'Removes expired account_activation rows, once a day'
    DO
		DELETE FROM veryrezsi.account_activation WHERE expiration < now();